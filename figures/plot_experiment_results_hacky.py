...
# hack to get some extra reps
run_data = list(Path("../data/finalrun/").glob("*out.csv"))
run2_data = list(Path("../data/largerun1").glob("*out.csv"))

def compile_results(run_data, run2_data):
    dfiles_run1 = pd.concat([pd.read_csv(d) for d in run_data])
    dfiles_run2 = pd.concat([pd.read_csv(d) for d in run2_data])
    
    # run2 had 10 excessive trials, remove them
    dfiles_run2 = dfiles_run2[dfiles_run2.trial_id < 750]
    dfiles_run2['trial_id'][np.logical_and(dfiles_run2.trial_id == 500, dfiles_run2.problem=="discrim_lowdim")] = -1
    dfiles_run2 = dfiles_run2[dfiles_run2.trial_id > 0]

    # increment overlapping rep counts
    dfiles_run2['rep'] = dfiles_run2['rep'] + dfiles_run1.rep.max() + 1
    df = pd.concat([dfiles_run1, dfiles_run2])
...
def make_benchmark_figure():
    res, itrs = compile_results(run_data, run2_data)
